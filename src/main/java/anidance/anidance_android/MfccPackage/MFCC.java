...
public class MFCC {
    ...
    public float[] process(double[] doubleInputBuffer) {
        ...
        Log.d(TAG, "flag2");
        Log.d(TAG, "mfccResult.shape = (" + mfccResult.length + ", " + mfccResult[0].length + ")");
        float[] finalResult = finalshape(mfccResult);
        Log.d(TAG, "flag3");
        return finalResult;
    }
    */
    ...
    private double[][] dctMfcc(double[] y) {
        double[][] melRes = melSpectrogram(y);
        final double[][] specTroGram = powerToDb(melRes);
    ...
    private double[][] melSpectrogram(double[] y) {
        ...
        */
        MyMatmul matmul = new MyMatmul(melBasis, spectro);
        double[][] melS = matmul.getResult();
        return melS;
    }
